...
/**
 * CP_Diet_Planner_Plan
 *
 * Plan Singleton Class
 *
 * @since	1.0.0
 *
 * @param	void
...
if( ! class_exists('CP_Diet_Planner_Plan') ) :

class CP_Diet_Planner_Plan {
	/** @var wp post of view */
	var $post = null;
	...
	function init( $post ) {
		...
		if ( $post && CP_DIET_PLANNER_POST_TYPE == get_post_type( $post ) ) {
			$this->id = $post->ID;
		...
		do_action( 'cp_diet_planner_plan_init', $this );
		return $this;
	}
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'cp_diet_planner_plan_properties',
			$properties, $this );
	...
	public function get_shortcode( $args = '' ) {
		...
		$shortcode = sprintf( '[cp-diet-planner-plan id="%1$d" title="%2$s"]',
				$this->id(), $title );

		return apply_filters( 'cp_diet_planner_shortcode',
			$shortcode, $args, $this );
	}
